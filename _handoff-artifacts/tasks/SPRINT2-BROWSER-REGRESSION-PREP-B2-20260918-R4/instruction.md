...
recovery: PM_FAILOVER_STALE_PREPARED_R3_NO_ACTIVE_OR_TERMINAL_0002

## Objective
Use B2 while Cursor A owns the current product/source repair. Prepare and validate browser/Playwright regression acceptance needed to immediately verify A's fix, without editing A-owned product implementation surfaces. This R4 supersedes stale PREPARED R3.
...
## Terminal
Publish `_handoff-artifacts/results/SPRINT2-BROWSER-REGRESSION-PREP-B2-20260918-R4/result.md` with changed files, commands, pass/fail counts, HEAD/worktree evidence, exact covered product gaps, and exact reacceptance command to run after A terminal.
