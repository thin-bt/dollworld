...
- blueprint commit: `250ba900945ef01a9f125fef268d4148da9abd5b`
...
4. Make the result immediately readable: winner/outcome first, then end reason/judge/range and final participant resources. The r2 blueprint deliberately puts the four result facts directly under the winner and the final-state comparison immediately after them.
5. Rework detailed log into a turn-oriented human-readable timeline. Preserve all contract-backed action information; requested/resolved differences and diagnostics may remain secondary where they are developer-facing. The turn header should make range transition readable without repeating technical payloads in every row.
...
11. Treat the r2 mock's tabs as information hierarchy, not a requirement to invent routing/state. Existing result/log loading behavior remains authoritative; anchors or a single continuous page are acceptable when that better preserves current contracts.
...
- start the real app and compare `/mock-battle` and `/mock-battle/result` against blueprint r2 at desktop and narrow viewport;
