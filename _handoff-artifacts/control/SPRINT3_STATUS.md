...
| S03-005 TE-011 regression publication pristine root gate + production web build | `SPRINT3-S03-005-TE011-PUBLICATION-GATE-A-20260923-R1` **PASS** | **`1973/1973`** (**137/137** files), wiki **58**, harness **2/2**, web build **PASS** @ product **`d62778c`** (TE-011 sidecar `discipleCount` → `applyTrainStat` lock; parent product **`ae23fb9`**) — **live release-gate binding** |

**Current-master product bytes on canonical `origin/master`:** latest `apps/` + `packages/` commit **`d62778c`** (S03-005 TE-011 regression test lock; parent product **`ae23fb9`**). Control-only commits may advance tip without changing product SHA.
...
- **Live release-gate binding:** terminal pristine root gate **`SPRINT3-S03-005-TE011-PUBLICATION-GATE-A-20260923-R1` **PASS** @ product **`d62778c`** (**1973/1973**, **137/137** files). Historical post–F-02 gate @ **`ae23fb9`**, post–E2A9 gate @ **`a3776c1`**, **S03-072** @ **`fdeed36`**, and post-WF14 Prettier gate @ **`7004411`** remain historical evidence only for earlier lineages.
- **S03-005 spec-source audit (terminal, local regression published this run):** `SPRINT3-S03-005-CURRENT-MASTER-EFFICIENCY-SPEC-SOURCE-AUDIT-A-20260923-R1` **PASS** → TE-011 canonical @ **`d62778c`**; does not assign **CLOSED**.
