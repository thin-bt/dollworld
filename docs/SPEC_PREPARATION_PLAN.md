...
| ミニ仕様 | 状態 |
|---|---|
| 08 人物能力・成長 | 作成・受入監査済み（`S1-SPEC-0.1.12`） |
| 09 技データ・習得 | 作成・受入監査済み（`S1-SPEC-0.1.12`） |
| 10 週間行動・訓練・習得処理 | 作成・受入監査済み（`S1-SPEC-0.1.12`） |
| 11 戦闘開始状態 | 作成・受入監査済み（`S1-SPEC-0.1.12`） |
| 12 戦闘ターン解決 | 作成・受入監査済み（`S1-SPEC-0.1.12`） |
| 13 戦闘終了・結果・ログ | 作成・受入監査済み（`S1-SPEC-0.1.12`） |
| 14 Sprint 1共通設定スキーマ | 作成・受入監査済み（`S1-SPEC-0.1.12`） |
...
|---|---|
| 正本`SPEC-0.1.2` | 確定 |
| ミニ仕様08〜14（`S1-SPEC-0.1.12`） | 仕様定義済み・受入監査済み（週間処理契約を`S1-SPEC-0.1.12`で明文化） |
| LLM Wiki同期 | 同期済み |
...
| Sprint 1全体 | **未完了**（S01-005以降は未着手） |

次の実装着手タスクは **S01-005**（戦闘開始入力adapter）である。S01-004の週間Processorは純粋関数として実装済みであり、WorldEngineへの登録は **S01-008** で行う。
