...
| ミニ仕様 | 状態 |
|---|---|
| 08 人物能力・成長 | 作成・受入監査済み（`S1-SPEC-0.1.15`） |
| 09 技データ・習得 | 作成・受入監査済み（`S1-SPEC-0.1.15`） |
| 10 週間行動・訓練・習得処理 | 作成・受入監査済み（`S1-SPEC-0.1.15`） |
| 11 戦闘開始状態 | 作成・受入監査済み（`S1-SPEC-0.1.15`） |
| 12 戦闘ターン解決 | 作成・受入監査済み（`S1-SPEC-0.1.15`） |
| 13 戦闘終了・結果・ログ | 作成・受入監査済み（`S1-SPEC-0.1.15`） |
| 14 Sprint 1共通設定スキーマ | 作成・受入監査済み（`S1-SPEC-0.1.15`） |
...
|---|---|
| 正本`SPEC-0.1.2` | 確定 |
| ミニ仕様08〜14（`S1-SPEC-0.1.15`） | 仕様定義済み・受入監査済み（移動状態補正を`S1-SPEC-0.1.15`で明文化。ターン入力契約は`S1-SPEC-0.1.14`、MatchId generator契約は`S1-SPEC-0.1.13`、週間処理契約は`S1-SPEC-0.1.12`で明文化済み） |
| LLM Wiki同期 | 同期済み |
...
次の実装着手タスクは **S01-006**（ターン解決）。S01-001〜S01-005は実装済み。移動状態補正（`moverStateModifier`／`opponentStateModifier`）は `S1-SPEC-0.1.15` で明文化済み。ターン入力契約（`replacementReason`／`battle-action-script-0.1.0`／技使用回数）は `S1-SPEC-0.1.14` で明文化済み。MatchId generator契約は `S1-SPEC-0.1.13` で明文化済みで、production実装はS01-005で完了した。S01-004の週間ProcessorとS01-005の戦闘開始はいずれも純粋関数として実装済みであり、WorldEngineへの登録は **S01-008** で行う。
