...
- 標準World処理では41歳の世界年を現役最終年として扱い、42歳到達時の設定年初月第1週年初処理で強制的に`retired`へ遷移する（既定1月）。42歳以上は週間Plannerの正式訓練・公式戦対象外とし、成長係数は0とする
- 年齢0..7および42歳以上は `10-training-and-learning.md` §3.0 の週間行動パイプライン非適格であり、週間Planner／候補生成／週間訓練action履歴へ入らない（週間rest参加者ではない）
