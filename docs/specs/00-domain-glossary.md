...
| 初期NPC | 世界1年・設定年初月第1週（既定1月）の開始時点から存在するNPC |
| ユーザー始祖 | ユーザーが世界へ投入する家系の起点。年途中の申請は次年度・設定年初月第1週から参加 |
| 家系 | 人物の所属家と歴史。生物学的父母とは別に所属家系を持つ |
| 血統 | 父母から子へ続く生物学的系譜 |
| 流派 | 技・型・術・修行方法・戦い方を共有する体系 |
| 師系 | 師匠から門下へ続く技術上の系譜 |
| 世界年 | 設定年初月第1週から直前月第4週までの48週間（既定：1月第1週〜12月第4週） |
| 通算世界週 | 世界1年・設定年初月第1週を0とする週番号（既定：世界1年1月第1週） |
| 出生時期 | 全人物を設定年初月第1週生まれとして扱う（既定1月）。人物別の誕生月・誕生週・誕生日は保持しない |
| ランク | F〜Sの到達階級。降格しない |
...
人物ごとの誕生月・誕生週・誕生日という用語・データは使用しない。全人物は設定年初月第1週生まれ（既定1月）。
...
Sprint 1以降に開始する新規runの`simulationId`は、02ミニ仕様の`SimulationIdentity`（schemaVersion `0.5.0`）から生成する。正本仕様版は`SPEC-0.1.3`、Sprint 1仕様版は`S1-SPEC-0.1.21`をidentityへ含める。`initialWeeklyTrainingSidecarHash`に加え`worldCalendarConfigHash`／`yearStartProcessorManifestHash`を必須材料とする。詳細は`docs/specs/02-config-schema.md`および`docs/specs/14-sprint1-config-schema.md`を参照する。
...
- 世界年の設定年初月第1週に、その世界年より前に出生した`lifeStatus=living`かつ`participationStatus=active`の人物を一斉に1歳加齢する。
- `waiting`、`stopped`、`deceased`、および同じ年初処理で出生した年齢0歳の人物は加齢しない。
- 同一世界年に複数回加齢してはならない。
- 初期世界は世界1年・設定年初月第1週の年初処理後スナップショットとする（既定1月）。
