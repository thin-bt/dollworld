...
- 大会日程を含む行動評価
- 模擬戦・合同修行（正本「週間修行の種類」）はSprint 1の`WeeklyAction`に含めない。戦闘エンジンの`battleKind=mock`とは別概念で、週間行動との接続は後続Sprintとする。Sprint 1の週間疲労設定には`mockBattle`キーを置かない
- 間合い・精神訓練（正本「週間修行の種類」）はSprint 1の`WeeklyAction`に含めない。速度・技量・精神を対象とする専用週間メニューは後続Sprintとする
- 独自技研究（正本「週間修行の種類」「独自技の発生」）はSprint 1の`WeeklyAction`に含めない。研究値蓄積・生成判定は09仕様の後続Sprint事項と接続する
- 師匠変更
- 恋愛・関係変化
- 引退判断
- 指導行動
