...
Sprint 1以降に開始する新規runでは、設定ハッシュ・seed・仕様版に加え、Sprint 1の決定的ルール入力を含む`SimulationIdentity`から`simulationId`を生成する。Sprint 1仕様版は`S1-SPEC-0.1.17`。旧`S1-SPEC-0.1.16`を新規runの現行Sprint 1 identityとして受理しない。Sprint 0で生成済みの旧`simulationId`を再計算して置換しない。
...
- specVersions:
    ...
    - { specSetId: "sprint1", version: "S1-SPEC-0.1.17" }
- rngAlgorithmVersion
