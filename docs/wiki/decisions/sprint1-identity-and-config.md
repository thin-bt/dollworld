...
- SimulationIdentity（schemaVersion `0.4.0`、`initialWeeklyTrainingSidecarHash`必須）
- `specVersions` の例:
  - main: `SPEC-0.1.2`
  - sprint0: `S0-SPEC-0.1.5`
  - sprint1: `S1-SPEC-0.1.20`
...
- 戦闘開始`sourceSnapshot` baselineにより戦闘中も`sourceSnapshotHash`を常時検証する。BattleState schema現行は`0.6.0`（14・11／`S1-SPEC-0.1.17`）。Sprint1 identity現行版は`S1-SPEC-0.1.20`
- CLI新規optionは`--sprint1-input`のみ（`Sprint1CliInput` 0.1.0）。path／mtimeはidentity材料にしない
- run-metadata Sprint1 new-runは`0.4.0`。initial-world `0.4.0`（`initialWeeklyTrainingSidecarSnapshot`投影）／final-world `0.3.0`（`weeklyTrainingSidecars`投影）
...
- current `validateSimulationIdentity` は 0.4.0 のみ。0.3.0 専用legacy readerは存在せず新設しない
