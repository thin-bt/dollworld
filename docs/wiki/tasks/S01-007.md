...
- 実装状態: 未着手（pending）。production BattleResult／finalize／runBattleToCompletionは未実装
- 契約ヘルパー: `battle-result-contracts.ts`（0.1.18）／`battle-execution-abort.ts`（0.1.19）
- `RunBattleToCompletionResult`は3 kindのみ。start後dependency／invariant failureは`BattleExecutionAbortError` throw
...
該当なし。本タスク実装が次。WorldEngine接続はS01-008。
