...
- 依存: S01-006
- 仕様版: `S1-SPEC-0.1.18`（BattleResult決定的契約clarification済み）
- 実装状態: 未着手（pending）。production BattleResult／finalizeは未実装
- 契約ヘルパー: `packages/simulation-core/src/sprint1/battle-result-contracts.ts`（phase／tie-break／masteryの純関数のみ）
...
- `packages/simulation-core/src/sprint1/battle-result-contracts.ts`（0.1.18契約純関数。finalizeBattleResultは未実装）
