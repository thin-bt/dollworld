...
S01-006 productionターンResolverは**実装済み**。`S1-SPEC-0.1.16` の`movementChance`はproduction実装済み。`S1-SPEC-0.1.17` の`sourceSnapshotHash`常時検証と`sourceSnapshot`＋`BattleDetailedLog` replay validator（**意味再計算済み**／`battleSeed`連続RNG）も実装済み。`S1-SPEC-0.1.19`はBattleResult決定的契約clarification（S01-007実装前）。本ページはそれらの索引である。次はS01-007。Sprint 1全体は未完了。BattleResultは未実装。
